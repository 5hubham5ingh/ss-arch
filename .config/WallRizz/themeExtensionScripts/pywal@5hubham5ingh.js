...
export function setTheme(
  ...
  const nvimFile = STD.open(pywalCacheDir.concat("/colors-wal.vim"), "w+");
  nvimFile.puts(themeConfig["colors-wal.vim"]);
  nvimFile.close();

  // update different applications
...
function generateConfigFiles(theme) {
  ...
  const colorsJson = JSON.stringify(
    {
      wallpaper: theme.wallpaper || "/dummy/path/to/wallpaper.jpg",
      special: {
  ...
  // Generate the colors-wal.vim file (Vim script format)
  const colorsVim = `
" Special
let wallpaper  = "/dummy/path/to/wallpaper.jpg"
...
${
    colorKeys
      .map((key, index) => `let ${key} = "${colors[index]}"`)
      .join("\n")
  }
  `.trim();
